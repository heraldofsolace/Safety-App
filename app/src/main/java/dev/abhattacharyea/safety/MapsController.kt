...
class MapsController(context: Context, googleMap: GoogleMap) {
	...
	private val mTimeSquare = LatLng(22.5726, 88.3639)
	...
	fun setCustomMarker() {
		...
		val markerOptions: MarkerOptions =
			MarkerOptions().position(mTimeSquare).title(mContext.getString(R.string.ic_kolkata))
	...
	fun setMarkersAndRoute(route: Route) {
		...
		val startMarkerOptions: MarkerOptions =
			MarkerOptions().position(startLatLng).title(route.startName)
				.icon(BitmapDescriptorFactory.fromBitmap(MapsFactory.drawMarker(mContext, "S")))
